(function() {
	...
	angular.module('csvuploads', ['ui.router', 'ngCsvImport', 'user'])
	
	.controller('CsvUploadsController', [ '$scope', '$state', 'CsvUploadsService', 'CsvAlertsService', 'UserService', function($scope, $state, CsvUploadsService, CsvAlertsService, UserService) {
		...
		$scope.showSuccess = function(){
			if (CsvAlertsService.hasAlert()) {
				$scope.showSuccessAlert = true;
			  $scope.success = CsvAlertsService.getSuccess();
			  $scope.cb_total = CsvAlertsService.getTotal();
			  CsvAlertsService.reset();
			}
		};
		
		$scope.moreChargebacks = function(){
		...
		$scope.service = CsvUploadsService;
		...
		$scope.save = function() {
			...
			$scope.service.save( { 'user': $scope.user, 'chargebacks': $scope.chargebacks } ).then(function () {
				$scope.json = {};
				$scope.user = "";
				CsvAlertsService.setSuccess('Success!');
				CsvAlertsService.setTotal($scope.cb_total);
				$scope.showSuccess();
			}, function (res) {
				console.log(res);
			});
	...
	.factory('CsvUploadsService', ['$http', '$timeout', function ($http, $timeout) {
	...
	.factory('CsvAlertsService', function () {
	  var success = {},
	      total = 0,
	      alert = false;
	  return {
